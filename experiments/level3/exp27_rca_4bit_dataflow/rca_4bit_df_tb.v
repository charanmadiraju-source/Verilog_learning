`timescale 1ns/1ps
module rca_4bit_df_tb;
    reg [3:0] a,b; reg cin; wire [3:0] sum; wire cout;
    rca_4bit_df uut(.a(a),.b(b),.cin(cin),.sum(sum),.cout(cout));
    integer i,j,errors=0;
    initial begin
        $dumpfile("rca_4bit_df.vcd"); $dumpvars(0,rca_4bit_df_tb);
        ...
        if(errors==0) $display("PASS: 4-bit RCA (dataflow) test complete.");
